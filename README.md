...
- **[VERIFICATION.md](VERIFICATION.md)** — the **critic role** for this build. The W2 brief lists "Critic agent that rejects uncited claims or unsafe action suggestions" under Core; we fulfill it with a **deterministic verification gate** (citation enforcement strips uncited claims; the constraint-boundary rule catches advisory phrasing like "should monitor closely") rather than a separate critic-LLM. Defense in [W2_ARCHITECTURE.md §15](W2_ARCHITECTURE.md).
- **[Documentation/AgentForge/implementation/w2-cost-latency-report.md](Documentation/AgentForge/implementation/w2-cost-latency-report.md)** — brief deliverable §7: per-encounter unit economics, projected production cost at 100/1K/10K/100K clinicians, bottleneck analysis. Anthropic/Cohere/Langfuse console-pull tables (§2, §4.2-3) are post-submission roll; substantive economics are in §1, §3, §5, §6.
...
| [EVALUATION.md](EVALUATION.md)       | Eval suite: 12 deterministic check rules (10 W1 stop-the-line invariants + instructor-named failure modes + constraint-boundary "automation, not advice" gate, plus 2 W2 additions for extraction schema validity and citation substring fidelity), **88 curated cases** under 5 boolean rubric categories (`schema_valid`, `citation_present`, `factually_consistent`, `safe_refusal`, `no_phi_in_logs`), pinned baseline `w2-consolidated-2026-05-07`, plus a committed **LLM judge** (prompt + model checked in, opt-in `EVAL_RUN_JUDGE=1`) for `factually_consistent` + `safe_refusal` cases. |
