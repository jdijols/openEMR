...
**Caveat:** the in-process cost estimator at [agentforge/api/src/agent/cost_estimate.ts:5-9](agentforge/api/src/agent/cost_estimate.ts:5) uses heuristic rates per provider key, not per model. Anthropic is keyed at `$3 / $15` per million tokens, which is correct for Sonnet but not for Haiku 4.5 (which is `$1 / $5`). Langfuse Cloud computes the *correct* cost from its own model-price database keyed by the model name we pass it (`claude-haiku-4-5`), so the dashboard is accurate. The in-process estimate is what gets logged to the local `costMeta` line and is therefore in a 3x-too-high direction for the current default model. This is a known gap, called out in [Documentation/AgentForge/process/18-langfuse-observability-cost-analysis.md](Documentation/AgentForge/process/18-langfuse-observability-cost-analysis.md) §Follow-ups, and is on the post-doc fix list below.
...
The in-process `estimateUsdForProviderTokens` uses provider-key rates (Anthropic = `$3/$15`), not model-name rates (Haiku 4.5 = `$1/$5`). The Langfuse-side cost is correct because the cloud uses its model-price DB keyed on the canonical model name we pass via `getProviderModelId`. The local `costMeta` log is a 3x overestimate for the current default model. Discussed in [Documentation/AgentForge/process/18-langfuse-observability-cost-analysis.md](Documentation/AgentForge/process/18-langfuse-observability-cost-analysis.md) §Follow-ups.

---

## Open gaps for follow-up code work

- **Fix `cost_estimate.ts` Anthropic rates** — change from Sonnet-shape `$3/$15` to Haiku 4.5 actual `$1/$5` at [agentforge/api/src/agent/cost_estimate.ts:6](agentforge/api/src/agent/cost_estimate.ts:6). Also update the OpenAI Azure rate to match the deployment we're actually using; the current `$5/$15` is a generic GPT-4-class rate. ~5 minutes; flagged in process journal as a debt.
- **Add a one-pager runbook for "how to debug a prod issue from a correlation_id"** — under `Documentation/AgentForge/runbooks/observability-debug.md`, covering: how to extract the correlation_id from the conversation log, how to load the Langfuse trace, what to look for at each step (tool errors, verification events, cost outliers). Useful for instructor review and for future-Jason at 2am.
- **Wire a small set of canonical Langfuse queries** as pre-saved views: P95 tool latency, refusal-rate breakdown by category, cost per turn by use case. The data is already there; the queries aren't.
- **Add a redactor coverage matrix test** — a `redact.coverage.test.ts` enumerating known-PHI strings that must be redacted and known-non-PHI strings that must survive. Closes the documented over-redaction trade-off with regression coverage.
- **Module-level docblock additions** — same shape as the VERIFICATION.md follow-up: a top-of-file architectural statement on `observability/index.ts` naming the trace shape (trace per turn, span per tool, event per gate, generation per LLM call) so a future maintainer doesn't have to reverse-engineer the convention.
- **`/health` endpoint should actively probe Langfuse egress** — currently `/health` returns `langfuse: "unknown"` because it doesn't actually try to reach the configured base URL. A small change to issue a 200ms-timeout HEAD request would convert "unknown" into "ok / unreachable" and let the canary scripts catch a Langfuse outage.
