...
| 4 | **Eval Dataset** | 50 synthetic/demo cases with expected behavior, boolean rubrics, judge configuration, results | ✅ shipped (75 cases — exceeds the 50-case brief target after G2-Final-15 rebalance) | **75/75 cases** under [agentforge/api/eval/cases/curated/](../../agentforge/api/eval/cases/curated/) (50 G2-Early + 25 G2-Final-15 cohort-grounded rebalance — see [W2_ARCHITECTURE.md §11](../../W2_ARCHITECTURE.md) "75-case composition"). 5 boolean rubric categories (`schema_valid`, `citation_present`, `factually_consistent`, `safe_refusal`, `no_phi_in_logs`); per-category counts 10/10/12/35/8. Pinned baseline: [agentforge/api/eval/baseline.json](../../agentforge/api/eval/baseline.json) `version: w2-final-rebalance-2026-05-06`. Latest run: [agentforge/api/eval/reports/](../../agentforge/api/eval/reports/) (`cases:75 failures:0 gate_breaches:0`). Judge config (deterministic, no LLM-as-judge): [agentforge/api/eval/runner.ts](../../agentforge/api/eval/runner.ts). |
...
- [x] All vitest tests green (API-side): 292/293 + 1 pre-existing skip; CUI-side bbox.test 4/4
- [x] Eval suite 75/75 green (`npm run eval` from `agentforge/api/`)
- [x] Eval gate baseline pinned at `w2-final-rebalance-2026-05-06`; `gate_breaches_count: 0`
