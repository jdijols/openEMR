...
| 4 | **Eval Dataset** | 50 synthetic/demo cases with expected behavior, boolean rubrics, judge configuration, results | ✅ shipped | 50/50 cases under [agentforge/api/eval/cases/curated/](../../agentforge/api/eval/cases/curated/). 5 boolean rubric categories (`schema_valid`, `citation_present`, `factually_consistent`, `safe_refusal`, `no_phi_in_logs`). Pinned baseline: [agentforge/api/eval/baseline.json](../../agentforge/api/eval/baseline.json) `version: w2-early-2026-05-06`. Latest run: [agentforge/api/eval/reports/](../../agentforge/api/eval/reports/) (`cases:50 failures:0 gate_breaches:0`). Judge config (deterministic, no LLM-as-judge): [agentforge/api/eval/runner.ts](../../agentforge/api/eval/runner.ts). |
...
- **W2 Eval cases (50):** [agentforge/api/eval/cases/curated/](../../agentforge/api/eval/cases/curated/)
...
- [x] Eval suite 50/50 green (`npm run eval` from `agentforge/api/`)
- [x] Eval gate baseline pinned at `w2-early-2026-05-06`; `gate_breaches_count: 0`
