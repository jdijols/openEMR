...
/**
 * G2-Early-21 — medication discontinue (soft-delete) payload parser.
 *
 * @package   OpenEMR
...
final class MedicationDiscontinuePayload
{
    private const KEYS = ['medication_uuid'];

    private function __construct(
        private readonly string $medicationUuid,
    ) {
    }
    ...
    public static function parse(?array $payload): array
    {
        ...
        if (!isset($payload['medication_uuid']) || !is_string($payload['medication_uuid'])) {
            return [null, 'missing_uuid'];
        }

        $candidate = trim($payload['medication_uuid']);
    ...
    public function medicationUuid(): string
    {
