...
/**
 * G2-Final-11 — confirmed partial demographics update on patient_data.
...
use OpenEMR\Modules\AgentForge\Write\OpenEmrPatientDemographicsAdapter;
...
[$payload, $payloadError] = DemographicsUpdatePayload::parse(is_array($payloadRaw) ? $payloadRaw : null);
...
$action = new DemographicsUpdateAction(
    new OpenEmrPatientDemographicsAdapter(),
...
if ($result->isAccepted()) {
    AgentAuditLogger::recordAgentEvent(
        ...
        $correlationId,
        true,
        ['fields' => array_keys($payload->columnPatch())],
    );
