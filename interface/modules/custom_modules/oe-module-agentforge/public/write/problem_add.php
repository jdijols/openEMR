...
/**
 * W2 intake-bundle — confirmed medical problem add (`lists` insert).
 * write_target='problem_add'. Patient-scoped (no encounter binding).
 *
 * Mirrors `family_history_add.php`: parse → authorize (ChartContextGate +
 * AclMap) → resolve pid (fall back to PatientService probe if the session
 * doesn't carry one) → MedicalProblemAddAction → emit JSON outcome.
 *
 * @package   OpenEMR
...
use OpenEMR\Modules\AgentForge\Write\MedicalProblemAddAction;
use OpenEMR\Modules\AgentForge\Write\MedicalProblemAddPayload;
use OpenEMR\Modules\AgentForge\Write\MysqlCompletedWriteProposalLedger;
use OpenEMR\Modules\AgentForge\Write\OpenEmrPatientMedicalProblemAdapter;
...
[$payload, $payloadError] = MedicalProblemAddPayload::parse(is_array($payloadRaw) ? $payloadRaw : null);
...
if ($payloadError === 'invalid_payload' || $payload === null) {
    agentforge_emit_json(400, ['error' => 'invalid_request', 'correlation_id' => $correlationId]);
}
...
$action = new MedicalProblemAddAction(
    new OpenEmrPatientMedicalProblemAdapter(),
