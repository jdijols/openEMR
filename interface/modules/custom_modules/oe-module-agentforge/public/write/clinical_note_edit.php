...
/**
 * Confirmed clinical-note edit (update or soft-delete) — encounter-scoped, by note UUID.
 * Dispatches by payload.action so a single endpoint handles both write_target='clinical_note_update'
 * and write_target='clinical_note_delete' from the agent.
 *
 * @package   OpenEMR
...
use OpenEMR\Modules\AgentForge\Write\ClinicalNoteEditAction;
use OpenEMR\Modules\AgentForge\Write\ClinicalNoteEditPayload;
...
use OpenEMR\Modules\AgentForge\Write\OpenEmrClinicalNoteEditAdapter;
...
[$payload, $payloadError] = ClinicalNoteEditPayload::parse(is_array($payloadRaw) ? $payloadRaw : null);
$auditTarget = is_object($payload) && $payload->isDelete() ? 'clinical_note_delete' : 'clinical_note_update';

if ($payloadError === 'unsupported_payload' || $payloadError === 'invalid_action') {
    try {
...
if ($payloadError === 'missing_uuid' || $payloadError === 'missing_text' || $payload === null) {
    agentforge_emit_json(400, ['error' => 'invalid_request', 'correlation_id' => $correlationId]);
}

$auditTarget = $payload->isDelete() ? 'clinical_note_delete' : 'clinical_note_update';
...
$action = new ClinicalNoteEditAction(
    new OpenEmrClinicalNoteEditAdapter(new ClinicalNotesService()),
