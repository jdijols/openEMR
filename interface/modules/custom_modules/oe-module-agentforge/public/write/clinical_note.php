...
/**
 * Confirmed clinical-note write — appends physician dictation to the per-encounter
 * progress note via ClinicalNotesService. Distinct from chief_complaint.php so the
 * encounter reason (front-desk schedule field) is never overwritten.
 *
 * @package   OpenEMR
...
use OpenEMR\Modules\AgentForge\Write\ClinicalNoteWriteAction;
use OpenEMR\Modules\AgentForge\Write\ClinicalNoteWritePayload;
...
use OpenEMR\Modules\AgentForge\Write\OpenEmrClinicalNoteAdapter;
...
[$payload, $payloadError] = ClinicalNoteWritePayload::parse(is_array($payloadRaw) ? $payloadRaw : null);
...
if ($payloadError === 'empty_text' || $payload === null) {
    agentforge_emit_json(400, ['error' => 'invalid_request', 'correlation_id' => $correlationId]);
}
...
$action = new ClinicalNoteWriteAction(
    new OpenEmrClinicalNoteAdapter(new ClinicalNotesService()),
