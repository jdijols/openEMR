...
**Citation navigation (MVP):** When a displayed citation has a source pack with a supported navigation hint, the CUI sends a `postMessage` to the OpenEMR parent shell. The module-owned host script maps that bounded source metadata (resource family, ids, retrieval path) to one or two known chart destinations for MVP. Unsupported resource types fall back to safe chart-level navigation until their specific surfaces are implemented. Navigation stays inside the authenticated OpenEMR session and must not open a different patient than the active chart.
